...
open class Client: NSObject, NSCoding, Credential {
    ...
    open internal(set) var activeUser: User? {
        willSet (newActiveUser) {
            if let activeUser = newActiveUser {
                ...
                if let sharedKeychain = sharedKeychain, let socialIdentity = activeUser.socialIdentity {
                    sharedKeychain.kinveyAuth = socialIdentity[.kinvey]
                }
    ...
    open var micApiVersion: MICApiVersion? = .v1 {
        didSet {
            KCSUser.setMICApiVersion(micApiVersion?.rawValue)
        }
    }
    ...
    /// Initialize a `Client` instance with all the needed parameters and requires a boolean to encrypt or not any store created using this client instance.
    open func initialize(appKey: String, appSecret: String, accessGroup: String? = nil, apiHostName: URL = Client.defaultApiHostName, authHostName: URL = Client.defaultAuthHostName, encrypted: Bool, schema: Schema? = nil, completionHandler: User.UserHandler) {
    ...
    /// Initialize a `Client` instance with all the needed parameters.
    open func initialize(appKey: String, appSecret: String, accessGroup: String? = nil, apiHostName: URL = Client.defaultApiHostName, authHostName: URL = Client.defaultAuthHostName, encryptionKey: Data? = nil, schema: Schema? = nil, completionHandler: @escaping User.UserHandler) {
        validateInitialize(appKey: appKey, appSecret: appSecret)
        ...
        if let user = keychain.user {
            ...
            User.login(authSource: .kinvey, kinveyAuth, client: self, completionHandler: completionHandler)
        } else {
