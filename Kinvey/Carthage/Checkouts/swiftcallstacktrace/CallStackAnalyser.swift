...
class CallStackAnalyser {
    ...
    private static func cleanMethod(method:(String)) -> String {
        if (method.characters.count > 1) {
            ...
            if (firstChar == "(") {
                return method.substringWithRange(method.startIndex.advancedBy(1) ..< method.endIndex.advancedBy(0))
            }
        }
        return method
    }
    ...
    static func classAndMethodForStackSymbol(stackSymbol:String, includeImmediateParentClass:Bool? = false) -> (String,String)? {
        ...
        if (components.count >= 4) {
            ...
            if (numberOfComponents >= 2) {
                ...
                if let includeImmediateParentClass = includeImmediateParentClass {
                    if (includeImmediateParentClass && numberOfComponents >= 3) {
